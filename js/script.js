...
document.addEventListener('keydown', e => {
	if (e.repeat) {
		// so the sound doesn't play multiple times when you hold down the key, in other words, the function doesn't run again while key is pressed down bc e.repeat is true
		return;
	}
...
let playSound = key => {
	...
	audio.play().then(() => audio.remove()); // every time a key is pressed a new audio element is created so it needs to be deleted after the sound has played
};
